...
def getitem(M, k):
    ...
    if not (k[0] in M.D[0] and k[1] in M.D[1] and (k[0], k[1]) in M.f):
        return 0
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, {(x1, y1) : x*getitem(M, (x1,y1)) for x1 in M.D[0] for y1 in M.D[1]})
...
def vector_matrix_mul(v, M):
    ...
    return Vec(M.D[1], {x2: v * Vec(M.D[0], {x:getitem(M, (x, x2)) for x in M.D[0]}) for x2 in M.D[1]})
...
def matrix_vector_mul(M, v):
    ...
    return Vec(M.D[0], {x2: v * Vec(M.D[1], {x:getitem(M, (x2,x)) for x in M.D[1]})  for x2 in M.D[0]})
...
def matrix_matrix_mul(A, B):
    ...
    #return Mat((A.D[0],B.D[1]), {(0,0): (Vec(A.D[0], {x:getitem(A, (x, y)) for x in A.D[0]})*B.__getitem__(0) for y in A.D[1]) })
    #return Mat((A.D[0],B.D[1]), { (x,y) : Vec(A.D[x], {})*B for x in A.D[0] for y in B.D[1] })
    return Mat((A.D[0], B.D[1]), {(x, y): Vec.__getitem__(mat2rowdict(A)[x] * B,y) for x in A.D[0] for y in B.D[1]})
